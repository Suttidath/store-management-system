...
| id | INT (PK, identity) | |
| username | VARCHAR | |
| password | VARCHAR | ⚠️ เก็บเป็น plain text |
| date_register | DATE | |
...
### 🔴 สำคัญ (ควรทำก่อน)
1. **รหัสผ่านเก็บเป็น plain text** — ตาราง `users` เก็บรหัสตรงๆ ถ้า DB รั่วคือเห็นหมด
   → ควร hash (BCrypt / PBKDF2 / อย่างน้อย SHA-256 + salt)

### 🟡 ควรมีในระบบสต๊อกจริง
3. **แจ้งเตือนของใกล้หมด (low stock)** — เพิ่มฟิลด์ `min_quantity` ในสินค้า แล้วไฮไลต์/เตือนเมื่อ `quantity <= min_quantity`
4. **แก้ไข/ลบรายการใน Log ไม่ได้** — ตอนนี้ประวัติแก้ไม่ได้เลย (ดีต่อ audit) แต่ถ้าบันทึกผิดจะแก้ไม่ได้ → อาจเพิ่มระบบ "ยกเลิกรายการ" (สร้าง movement ย้อนกลับ) แทนการลบตรงๆ
5. **Export รายงาน** เป็น Excel/CSV/PDF จากหน้า Log

### 🟢 คุณภาพโค้ด / ประสบการณ์ใช้งาน
6. **ไฟล์ Designer ห้ามเปิดแก้พร้อมกับที่มีการแก้บนดิสก์** — Visual Studio ที่เปิดค้างอาจเขียนทับ (เคยเกิดในโปรเจกต์นี้แล้ว) → แก้ทีละที่แล้ว Save ทันที
7. **ยังไม่มีระบบสิทธิ์ (role)** — ทุกคนที่ล็อกอินทำได้ทุกอย่าง → ถ้าจะใช้จริงหลายคน ควรมี admin / staff
8. **Soft delete** — ปัจจุบันลบสินค้า/หมวดหมู่ที่มีการอ้างอิงไม่ได้เลย ทางเลือกที่ดีกว่าคือเพิ่มฟิลด์ `is_active` แล้วซ่อนแทนการลบ

### สรุป
โครงสร้างหลักครบและใช้งานได้จริงแล้ว (CRUD + สต๊อกแบบมี transaction + ประวัติ)
เรื่องที่ **ควรรีบแก้ที่สุดคือข้อ 1 (hash รหัสผ่าน)** เพราะกระทบความปลอดภัยโดยตรง
